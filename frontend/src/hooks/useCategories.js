import { getCategoriesRequest, getCategoryByIdRequest } from '../api/categoriesApi'
const useCategoriesHook = ()  =>{
  ...
  categoryImageMap.set(4, process.env.PUBLIC_URL + '/img/painting.jpeg');
  categoryImageMap.set(5, process.env.PUBLIC_URL + '/img/teaching.jpeg');
  categoryImageMap.set(6, process.env.PUBLIC_URL + '/img/cleaning.jpeg');
  ...
  const getCategories = async () => {
    ...
    return response.data.map(category => ({...category,image:categoryImageMap.get(category.id)}));
  }
