...
SELECT full_post.post_id,
       ...
       (SELECT COUNT(*)
        ...
        WHERE post_id = full_post.post_id) as post_contract_count,
       (SELECT COUNT(contract_id)
        FROM full_contract
                 NATURAL JOIN review
        WHERE post_id = full_post.post_id) as post_reviews_size
FROM full_post
...
GROUP BY full_post.post_id, post_title, post_available_hours, post_job_type, post_is_active, user_id, user_email,
         ...
         reviews, post_creation_date, user_creation_date, package_price, pi.image_id, pi.image_data, pi.image_type,
         post_contract_count, post_reviews_size
